...
Reading route:
1. Start with `test_train_residual_models_exports_three_bundles_with_shared_features()`.
2. Then inspect the synthetic telemetry rows to see the minimum shared schema.
3. Then focus on the `train_residual_models(...)` call.
...
from learning.train_residual import train_residual_models  # Import the public single-entry trainer under test.


class TrainResidualModelsTests(unittest.TestCase):  # Group public-trainer tests together.
    """Check that the public single-entry trainer exports three shared-feature bundles."""

    def test_train_residual_models_exports_three_bundles_with_shared_features(self) -> None:
        ...
        temp_path = tests_dir / "_train_residual_case"  # Use a dedicated temporary directory for this test.
        ...
        try:  # Ensure temporary artifacts are removed even if assertions fail.
            ...
            manifest = train_residual_models(
